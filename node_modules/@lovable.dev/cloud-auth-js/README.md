...
Legacy OAuth broker library for Lovable projects.
...
### Sign in with Lovable button

Import the first-party stylesheet and apply its class to the button that invokes the Lovable provider:

```tsx
import "@lovable.dev/cloud-auth-js/styles.css";
<button type="button" className="lovable-auth-button lovable-auth-button--full-width" onClick={handleLovableSignIn}>
  Continue with Lovable
</button>;
```

Use the `signInWithOAuth` flow above with `"lovable"` in `handleLovableSignIn`. Add
`lovable-auth-button--dark` on dark surfaces. The stylesheet owns the official mark, provider spacing, colors, focus
...
| `supportedOAuthOrigins` | `string[]` | `["https://oauth.lovable.app", "https://lovable.dev"]` | Allowed origins for OAuth postMessage |
...
| ---------------------- | ------------------------ | ---------------------------------------------------------- |
| `provider`             | `"google" \| "apple" \| "microsoft" \| "lovable"` | OAuth provider                                             |
